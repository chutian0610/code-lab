...
import info.victorchu.columnstorage.buffer.Buffer;
...
public final class XxHash64
{
    ...
    public static long hash(Buffer data)
    {
        return hash(data, 0, data.length());
    }

    public static long hash(long seed, Buffer data)
    {
        return hash(seed, data, 0, data.length());
    }

    public static long hash(Buffer data, int offset, int length)
    {
        return hash(DEFAULT_SEED, data, offset, length);
    }

    public static long hash(long seed, Buffer data, int offset, int length)
    {
        PreConditions.checkFromIndexSize(offset, length, data.length());
    ...
    public XxHash64 update(Buffer data)
    {
        return update(data, 0, data.length());
    }

    public XxHash64 update(Buffer data, int offset, int length)
    {
        PreConditions.checkFromIndexSize(offset, length, data.length());
